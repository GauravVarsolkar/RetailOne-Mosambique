...
class ForgotPinActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        forgotpinviewmodel.otpLivedata.observe(this){
            ...
            if(it.status == 1){
                ...
                binding.verifyOtp.text = getString(R.string.forgot_enter_otp_btn)
    ...
    private fun validateNewPin(newpin: String, confirmpin: String) {
        if(newpin.trim().length == 6){
            if(confirmpin == newpin){
                ...
                showMessage(getString(R.string.forgot_confirm_pin_mismatch))
            }
        }else{
            showMessage(getString(R.string.forgot_enter_valid_pin))
        }
    }
    ...
    private fun verifyMyOTP() {
        ...
        if(my_otp.trim().length==6){
            ...
            showMessage(getString(R.string.forgot_invalid_otp))
        }
    ...
    private fun validateMobile(mobileno: String) {
        ...
        if(mobileno.isEmpty() || mobileno.isBlank() || mobileno.length!=10){
            showMessage(getString(R.string.forgot_enter_valid_mobile))
        }else{
            sendOTP(mobileno)
            ...
            otpinput[i]?.addTextChangedListener(object : TextWatcher {
                ...
                override fun afterTextChanged(s: Editable) {
                    ...
                    if (otpinput[0]!!.text.toString().isNotEmpty() && otpinput[1]!!.text.toString().isNotEmpty()
                        ...
                        binding.verifyOtp.text = getString(R.string.forgot_verify_otp)
