    ...
    //class ReturnSaleActivity : BaseActivity(), OnDeleteItemClickListener,
    //    OnQuantityChangeListener {

    class ReturnSaleActivity : BaseActivity(),OnReturnQuantityChangeListener {

        lateinit var  binding : ActivityReturnSaleBinding
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            lifecycleScope.launch {
                ...
                //default search
               // returnsale_viewmodel.callReturnSalesDetailsApi(ReturnItemReq(invoice_id = "#241858938"),this@ReturnSaleActivity)
...
// Observe LiveData
            ...
            returnsale_viewmodel.salesListLiveData.observe(this) { response ->
                if (response.status == 1 && response.data.isNotEmpty()) {
                    showMessage(getString(R.string.sales_fetched_count_prefix, response.data.size))
                    ...
                    showMessage(getString(R.string.sales_no_sales_found))
                }
            }
...
// Call API
            ...
            binding.addcart.setOnClickListener {
                ...
                if(binding.searchBar.query.toString().trim()==""){
                    showMessage(getString(R.string.return_error_invalid_invoice))
                     ...
                     returnsale_viewmodel.callReturnSalesDetailsApi(ReturnItemReq(invoice_id = binding.searchBar.query.toString().trim()),this@ReturnSaleActivity)
           ...
           /* binding.addproductLayout.setOnClickListener {
                showMessage("Enter Invoice ID and Search off test")
            }*/
...
//dev
            returnsale_viewmodel.returnitem_liveData.observe(this){
                if(it.data.isNotEmpty()){
                    ...
                    intent.putExtra("invoice_id",  binding.searchBar.query.toString().trim()) // replace with actual invoice_id
                    startActivity(intent)

                }else{
                    showMessage(getString(R.string.return_error_no_invoice))
       ...
       /* private fun preparePositemRCV() {
        ...
        private fun showSucessDialog(msg: String, returnSaleRes: ReturnSaleRes) {
            ...
            confirm.setOnClickListener {
                ...
                val intent = Intent(this@ReturnSaleActivity, MPOSDashboardActivity::class.java)
                startActivity(intent)
                finish()
            }
