...
class SalesPaymentAdapter(private val salesList: List<Sale>, val context: Context
    ...
    override fun onBindViewHolder(holder: SalesPaymentViewHolder, position: Int) {
        ...
        val pt = item?.payment_type ?: ""
        holder.binding.txt4.text = when {
            pt.equals("Cash", true) -> context.getString(R.string.cashup_cash)
            pt.equals("M-Money", true) -> context.getString(R.string.cashup_mmoney)
            else -> pt
        }
