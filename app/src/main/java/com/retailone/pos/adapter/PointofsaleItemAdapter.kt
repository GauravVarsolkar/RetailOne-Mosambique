...
class PointofsaleItemAdapter(
    ...
    override fun onBindViewHolder(holder: POSItemViewHolder, position: Int) {
        ...
        val unitString = if (isLooseoil) context.getString(R.string.liters_label) else context.getString(R.string.units_label)
        holder.binding.itemUnit.text = "${FunUtils.DtoString(BatchUtils.getTotalPosQuantity(item.batch))} $unitString"
