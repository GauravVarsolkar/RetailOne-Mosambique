...
//    }
  ...
  /*  private fun updateReceivedQuantityX(batch_list: List<BatchReturnItem>,  position: Int) {
        for (item in batch_list) {
            // Check if the item is already in the returnbatchItemList based on sales_item_id
            val existingItemIndex = returnbatchItemList.indexOfFirst { it.sales_item_id == item.sales_item_id }
            ...
            if (item.batch_return_quantity > 0) {
                ...
                // If the batch_return_quantity is 0, remove the item if it exists
                if (existingItemIndex != -1) {
        ...
        onBatchChange(returnbatchItemList)
    }*/
    ////////
 /* private fun updateReceivedQuantityX(batch_list: List<BatchReturnItem>, position: Int) {
      for (item in batch_list) {
...
*/
  private fun updateReceivedQuantityX(batch_list: List<BatchReturnItem>, position: Int) {
      // ✅ Get the salesItemIds that belong to this product position
      val productItem = returnitem[0].salesItems[position]
      val productBatchIds = productItem.batches?.map { it.sales_item_id }?.toSet() ?: emptySet()

      // ✅ Remove ALL existing entries for this product first
      returnbatchItemList.removeAll { it.sales_item_id in productBatchIds }

      // ✅ Add back only entries with valid return quantity
      for (item in batch_list) {
          if (item.batch_return_quantity > 0) {
              returnbatchItemList.add(item)
          }
