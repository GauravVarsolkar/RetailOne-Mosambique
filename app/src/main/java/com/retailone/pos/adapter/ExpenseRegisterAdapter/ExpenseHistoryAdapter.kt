    ...
    override fun onBindViewHolder(holder: ExpenseHistoryViewHolder, position: Int) {
       ...
       // val datemodel = DateFormatter(item.pi_date).formatDateModel()
        ...
        holder.binding.apply {
            name.text = context.getString(R.string.expense_history_name) + (item.vendor?.vendor_name?:"-")
            category.text = context.getString(R.string.expense_history_category) + (item.category?.category_name?:"")
            price.text = context.getString(R.string.expense_history_amount) + formattedPrice

            item.created_at?.let {
                holder.binding.date.text = context.getString(R.string.expense_history_date) + DateTimeFormatting.formatOrderdate(it,localizationData.timezone)
            }
